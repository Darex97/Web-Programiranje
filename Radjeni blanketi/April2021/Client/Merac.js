export class Merac{
    ...
    iscrtajMerac(host){
        ...
        //let procenat=this.trenutna ovo je moje isti kurac
    ...
    update(value){
        ...
        fetch("https://localhost:5001/Ispit/DodajVrednost/"+this.id+"/"+value,{
            method:"PUT"
        }).then(data=>{
            data.json().then(info=>{
                this.trenutna=info.trenutnaVrednost;
                this.srednja=info.zbirIzmerenih/info.brojMerenja;
                this.max=info.maxVrednost;
                this.min=info.minVrednost;
